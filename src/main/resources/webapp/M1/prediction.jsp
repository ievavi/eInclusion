		...
		<div>
			...
			<div id="content" class="input-xxlarge"
				...
				<div>
					...
					<form id="formFromToDate" method="get"
						...
						<table align="center">
							<tr>
								<td>
									<div
										style="background-color: #f9f9f9; border: 1px solid; padding: 5px; border-color: #eee;">
										<%
										...
										<div>
											<label for="from"
												style="display: inline-block; width: 100px;">From*
												(Clickable)</label> <input type="text" id="from" style="width: 63px"
												name="from" value=<%=fieldFrom%>>
										</div>

										<div>
											<label for="to" style="display: inline-block; width: 100px;">To*
												(Clickable)</label> <input type="text" id="to" style="width: 63px"
												name="to" value=<%=fieldTo%>>
										</div>
								...
								<td>
									<div
										...
										<div>
											...
											<label for="fromM2"
												style="display: inline-block; width: 100px;">From*
												(0-100)%</label> <input type="text" id="fromM2" name="fromM2"
												onkeypress="return isNumberKey(event)"
												onchange="correctValueM(this)" maxlength=3
												style="width: 63px" value=<%=fieldFromM2%>>
										</div>

										<div>
											<label for="toM2"
												style="display: inline-block; width: 100px;">To*
												(0-100)%</label> <input type="text" id="toM2" name="toM2"
												onkeypress="return isNumberKey(event)"
												onchange="correctValueM(this)" maxlength=3
												style="width: 63px" value=<%=fieldToM2%>>
										</div>
										<button type="submit" style="width: 179px"
								...
								<td>
									<div
										...
										<div>
											...
											<label for="fromM3"
												style="display: inline-block; width: 100px;">From*
												(0-100)%</label> <input type="text" id="fromM3" name="fromM3"
												onchange="correctValueM(this)"
										...
										<div>
											<label for="toM3"
												style="display: inline-block; width: 100px;">To*
												(0-100)%</label> <input type="text" id="toM3" name="toM3"
												onkeypress="return isNumberKey(event)"
												onchange="correctValueM(this)" maxlength=3
												style="width: 63px" value=<%=fieldToM3%>>
										</div>
										<button type="submit" style="width: 179px"
					...
					<form>
						<div align="right">
							<button type="button" onclick="location.href = 'prediction.jsp';"
								style="width: 120px" class="btn btn-primary">Remove all
								filters</button>
						</div>
					</form>
										...
										<c:if
											...
											<tr class="toshow">
												...
												<c:choose>
													<c:when test="${m1 == 2}">
														<td class="green colored"
															onclick="showM1('<c:out value="${item.get(1)}" />','<c:out value="${item.get(2)}" />')"
															onmouseout="HideDef();" onmouseover="ShowDef();"><c:out
																value="${m1}" /></td>
													</c:when>
													<c:when test="${m1 == 1}">
														<td class="yellow colored"
															onclick="showM1('<c:out value="${item.get(1)}" />','<c:out value="${item.get(2)}" />')"
															onmouseout="HideDef();" onmouseover="ShowDef();"><c:out
																value="${m1}" /></td>
													</c:when>
													<c:when test="${m1 == 0}">
														<td class="red colored"
															onclick="showM1('<c:out value="${item.get(1)}" />','<c:out value="${item.get(2)}" />')"
															onmouseout="HideDef();" onmouseover="ShowDef();"><c:out
																value="${m1}" /></td>
													</c:when>
												...
												<c:choose>
													<c:when test="${m2 > 60}">
														<td class="green colored"
															onclick="showM2('<c:out value="${item.get(1)}" />','<c:out value="${item.get(2)}" />')"
															onmouseout="HideDef();" onmouseover="ShowDef();"><c:out
																value="${m2}" /></td>
													</c:when>
													<c:when test="${m2 > 25}">
												...
												<c:choose>
													<c:when test="${m3 > 60}">
														<td class="green colored"
															onclick="showM3('<c:out value="${item.get(1)}" />','<c:out value="${item.get(2)}" />')"
															onmouseout="HideDef();" onmouseover="ShowDef();"><c:out
																value="${m3}" /></td>
													</c:when>
													<c:when test="${m3 > 25}">
